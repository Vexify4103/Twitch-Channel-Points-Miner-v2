...
twitch_miner = TwitchChannelPointsMiner(
    username="vexi_fy",
    password="Pg%[4&gr]uQT%j+-",  # If no password will be provided, the script will ask interactively
    ...
    logger_settings=LoggerSettings(
        ...
        discord=Discord(
            webhook_api="https://discord.com/api/webhooks/1369048392314912939/CU-xXyVriu_mempM8fJzoi_a2NCcwPQTSyKdT-T12MSqsoa0fpKB34RVH3D8DKPTX4Qe",  # Discord Webhook URL
            events=[
                Events.STREAMER_ONLINE,
                Events.STREAMER_OFFLINE,
                Events.BET_START,
    ...
    streamer_settings=StreamerSettings(
        ...
        bet=BetSettings(
            ...
            delay=6,
            minimum_points=250000,  # Place the bet only if we have at least 20k points. Issue #113
            filter_condition=FilterCondition(
                by=OutcomeKeys.TOTAL_USERS,  # Where apply the filter. Allowed [PERCENTAGE_USERS, ODDS_PERCENTAGE, ODDS, TOP_POINTS, TOTAL_USERS, TOTAL_POINTS]
                where=Condition.GT,  # 'by' must be [GT, LT, GTE, LTE] than value
                value=20,
            ),
        ),
    ),
        ...
        Streamer(
            ...
            settings=StreamerSettings(
                ...
                bet=BetSettings(
                    ...
                    max_points=50000,
                    minimum_points=2000,
                    filter_condition=FilterCondition(by=OutcomeKeys.TOTAL_USERS, where=Condition.GT, value=20),
                ),
            ),
        ),
         Streamer(
